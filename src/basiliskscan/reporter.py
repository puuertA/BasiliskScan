...
class ReportGenerator:
    ...
    def _get_vuln_type(self, description: str) -> tuple:
        ...
        vuln_types = {
            'XSS': ('XSS', 'Cross-Site Scripting: permite injeção de código JavaScript malicioso em páginas web'),
            'RCE': ('RCE', 'Remote Code Execution: permite execução de código arbitrário no servidor'),
            'DoS': ('DoS', 'Denial of Service: causa indisponibilidade do serviço através de consumo excessivo de recursos'),
            'SQL Injection': ('SQL Injection', 'Injeção SQL: permite manipulação de consultas ao banco de dados'),
            'CSRF': ('CSRF', 'Cross-Site Request Forgery: força usuários autenticados a executar ações indesejadas'),
            'Path Traversal': ('Path Traversal', 'Travessia de Diretório: permite acesso a arquivos fora do diretório autorizado'),
            'Prototype Pollution': ('Prototype Pollution', 'Poluição de Protótipo: manipula protótipos de objetos JavaScript'),
            'Command Injection': ('Command Injection', 'Injeção de Comando: permite execução de comandos do sistema operacional'),
            'Information Disclosure': ('Information Disclosure', 'Vazamento de Informação: expõe dados sensíveis indevidamente'),
            'Auth Bypass': ('Auth Bypass', 'Bypass de Autenticação: contorna mecanismos de autenticação ou autorização'),
            'Security Issue': ('Security Issue', 'Problema de Segurança: vulnerabilidade de segurança geral')
        }
        
        if any(term in description_lower for term in ['xss', 'cross-site scripting', 'cross site scripting']):
            return vuln_types['XSS']
        elif any(term in description_lower for term in ['rce', 'remote code execution', 'code execution']):
            return vuln_types['RCE']
        elif any(term in description_lower for term in ['dos', 'denial of service', 'denial-of-service']):
            return vuln_types['DoS']
        elif any(term in description_lower for term in ['sql injection', 'sqli']):
            return vuln_types['SQL Injection']
        elif any(term in description_lower for term in ['csrf', 'cross-site request forgery']):
            return vuln_types['CSRF']
        elif any(term in description_lower for term in ['path traversal', 'directory traversal']):
            return vuln_types['Path Traversal']
        elif any(term in description_lower for term in ['prototype pollution']):
            return vuln_types['Prototype Pollution']
        elif any(term in description_lower for term in ['command injection']):
            return vuln_types['Command Injection']
        elif any(term in description_lower for term in ['information disclosure', 'information leak']):
            return vuln_types['Information Disclosure']
        elif any(term in description_lower for term in ['authentication', 'authorization']):
            return vuln_types['Auth Bypass']
        else:
            return vuln_types['Security Issue']
...
<body>
    <div class="container">
        ...
        <div class="header">
            ...
            <h1>🛡️ BasiliskScan - Relatório de Segurança</h1>
            <div class="subtitle">Análise de Componentes e Vulnerabilidades</div>
            
            <div class="scan-info">
                <div class="scan-info-item">
                    <div class="label">📅 Data da Análise</div>
                ...
                <div class="scan-info-item">
                    <div class="label">⏱️ Tempo de Execução</div>
        ...
        <div class="nav-tabs">
            ...
            <button class="nav-tab" onclick="openTab('dependencies', event)">
                📦 Dependências ({project_info['dependency_count']})
            </button>
            <button class="nav-tab" onclick="openTab('vulnerabilities', event)">
                🚨 Vulnerabilidades ({len(vulnerable_components)})
            </button>
        ...
        <div id="overview" class="tab-content active">
            <div class="section">
                <h2 class="section-title">📊 Visão Geral da Análise</h2>
                ...
                <h3 class="section-subtitle">🎯 Distribuição por Severidade</h3>
                <div class="stats-grid">
        ...
        <div id="dependencies" class="tab-content">
            <div class="section">
                <h2 class="section-title">📦 Dependências Identificadas</h2>
        ...
        for dep in dependencies:
            ...
            is_vulnerable = dep_name in vulnerabilities_data or any(k.lower() == dep_name.lower() for k in vulnerabilities_data.keys())
            status_badge = '<span class="severity-badge high">⚠️ Desatualizada</span>' if is_vulnerable else '<span class="severity-badge low">✅ Seguro</span>'
            
            ecosystem = dep.get('ecosystem', 'unknown')
            html_content += f'''
                            <tr>
                                <td><strong>{dep_name}</strong></td>
                                <td>{dep.get('version_spec', 'N/A')}</td>
            ...
            for comp in vulnerable_components:
                ...
                <div class="vuln-card {max_severity}">
                    ...
                    <div class="component-info">
                        <div class="info-item">
                            <div class="info-label">📌 Versão Instalada</div>
                        ...
                        <div class="info-item">
                            <div class="info-label">📄 Declarado em</div>
                    ...
                    html_content += f'''
                        <div class="vuln-item {severity}">
                            <div class="vuln-header">
                                ...
                                <span class="severity-badge {severity}">{severity_icon} {severity.upper()}</span>
                            </div>
                            ...
                            <div class="vuln-description">
                                ...
                                <div class="description-content" id="{expand_id}">
                                    ...
                                    <div class="description-translation">
                                        <div class="translation-label">🌐 Tradução (Português):</div>
                            ...
                            <div class="component-info">
                                <div class="info-item">
                                    <div class="info-label">✅ Versão Corrigida</div>
        ...
        <div id="recommendations" class="tab-content">
            <div class="section">
                <h2 class="section-title">💡 Recomendações de Mitigação</h2>'''
        ...
        if vulnerable_components:
            ...
            if critical_vulns:
                html_content += '''
                <h3 class="section-subtitle">🔴 Ações Urgentes (Críticas)</h3>'''
                ...
                <div class="recommendation-card">
                    <div class="title">🚨 Atualizar {comp_name} imediatamente</div>
                    <div class="content">
            ...
            if high_vulns:
                html_content += '''
                <h3 class="section-subtitle">🟠 Prioridade Alta</h3>'''
                for comp in high_vulns:
                    ...
                    for vuln in high_vulns_list:
                        ...
                        html_content += f'''
                <div class="recommendation-card">
                    <div class="title">⚠️ Atualizar {comp_name} em breve</div>
                    <div class="content">
                        <p><strong>Vulnerabilidade:</strong> {vuln.get('id', 'N/A')} (CVSS: {vuln.get('score', 0)})</p>
                        <p><strong>Ação:</strong> Atualizar para a versão {fixed_version}</p>
            ...
            html_content += '''
                <h3 class="section-subtitle">📋 Recomendações Gerais</h3>
                <div class="recommendation-card">
                    <div class="title">🔄 Mantenha suas dependências atualizadas</div>
                    <div class="content">
                ...
                <div class="recommendation-card">
                    <div class="title">🛡️ Implemente políticas de segurança</div>
                    <div class="content">
                ...
                <div class="recommendation-card">
                    <div class="title">📚 Monitore fontes oficiais</div>
                    <div class="content">
    ...
    def display_scan_results(self, dependencies: List[Dict], ecosystems: Dict, output_file: str, vulnerabilities: Optional[Dict[str, List[Dict]]] = None) -> None:
        ...
        self.console.print("[dim]💡 Dica: Abra o arquivo HTML no navegador para visualizar o relatório completo[/dim]")
